    ...
    def forward(
        ...
        for i in range(sequence_output.size()[0]):
            try:
                ...
                print(f"IndexError encountered at sequence {i}.")
                print(f"Details: {str(e)}")
                print(f"relations[{i}] length: {len(relations[i])} if available")
                print(f"sequence_output[{i}] shape: {sequence_output[i].shape}")
                print(f"Error in sequence batch: {[self.tokenizer.decode(input_ids[4], skip_special_tokens=True)]}")
                print(f"Error in sequence batch: {[self.tokenizer.decode(ids, skip_special_tokens=True) for ids in input_ids]}")
